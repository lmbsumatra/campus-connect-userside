...
const removeDatesAndDurations = async (postId, removedDates) => {
  for (const removedDate of removedDates) {
    // Find the date record by postId and date
    ...
    if (dateRecord) {
      // console.log(
      //   `Processing date ${removedDate} for removal for item ${postId}`
      // );
...
const updatePostById = async (req, res) => {
  ...
  try {
    ...
    if (removedDates.length > 0) {
      ...
      await removeDatesAndDurations(postId, removedDates);
    }
